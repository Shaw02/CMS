...
unsigned	int		AES::oid[] = {2,16,840,1,101,3,4,1,0};
...
//--------------------------------------------------------------
//	●引数
//		ObjectIdentifier*	ptOID
//	●返値
//					int		mode	暗号利用モード
//										-1	AESのOIDではない。
//==============================================================
int	AES::Check_OID(ObjectIdentifier* ptOID)
{
	int	i=0;
...
//--------------------------------------------------------------
//	●引数
//			unsigned char cMode,	暗号化モード
//			unsigned char Key[],	暗号鍵
//			unsigned char cIV[16]	初期化ベクタIV
//	●返値
//			無し
//==============================================================
void	AES::Set_AES(
			...
			__m128i	_xmm_IV)
{
	...
	switch(cMode){
		...
		case(42) :
			...
			break;
		default :
			errPrint("AES",": Unknown block cipher modes of operation");
			break;
	}

	//ASN.1の定義
	Set(oid,sizeof(oid)/sizeof(int));
...
void	AES::SetIV()
{
	vector = _mm_load_si128((__m128i *)IV.strValue.c_str());
}
...
void	AES::CBC_Cipher(void *data)
{
	...
	temp = AES_SSE_Cipher(Nr,w, _mm_xor_si128(temp, vector));
...
{
	__m128i	temp	= _mm_load_si128((__m128i*)data);
	__m128i	out		= _mm_xor_si128(AES_SSE_InvCipher(Nr,w,temp), vector);
